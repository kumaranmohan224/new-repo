...
public class MLG extends ScriptBase {
	StudentRegistrationController studcontroller=null;
	@Parameters({"ParameterNValue"})
	@Test(description = "MLG Tool", priority = 1, enabled = true)
	public void CreateCourse(String ParameterNValue) throws Exception
	{
		try {
			...
			this.strDTParametersNValues="InputDataRow=>"+MLGToolAsignment;
			createAssignment(MLGToolAsignment);
	...
	public void StudentTakingAssignment(String ParameterNValue) throws Exception
	{
		...
		this.strDTParametersNValues="InputDataRow=>6";
		//verifyQuestionMapWithoutAnswering();
		
		//Q1 MLG
		connect().TakeAssignmentPage().answerquestionOneMLG();
		...
		//Check 50% answered
		connect().TakeAssignmentPage().answerQuestionOneHalf();
		waiting(3);
		connect().TakeAssignmentPage().clickQuestionMap();
		connect().TakeAssignmentPage().checkStatusForQuestion("answered",1);
		waiting(5);
		...
		//Q2 MLG 50%
		connect().TakeAssignmentPage().answerQuestionTwoHalf();
		waiting(3);
		connect().TakeAssignmentPage().clickQuestionMap();
		connect().TakeAssignmentPage().checkStatusForQuestionTwo(2);
		waiting(5);
