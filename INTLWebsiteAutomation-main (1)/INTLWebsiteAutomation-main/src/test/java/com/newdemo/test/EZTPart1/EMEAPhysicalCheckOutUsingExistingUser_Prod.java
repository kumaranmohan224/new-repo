...
public class EMEAPhysicalCheckOutUsingExistingUser_Prod extends ScriptBase {
	...
	public void IntructorArea(String ParameterNValue, String env) throws Exception {
		try {
			...
			String strDescription = "Launch the EMEA URL in browser for EMEAPhysicalCheckOutUsingExistingUser_Prod";
	...
	public void loginLATAMURL() throws Exception {
		...
		connect().INTLLoginPage().clickActionPrintPrimaryToCartEMEA();
		connect().INTLLoginPage().clickAcceptAllButton();
		connect().INTLLoginPage().clickActionShowCartButton();
