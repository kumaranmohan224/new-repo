...
public class IRTTool  extends ScriptBase
{
	...
	public void CreateCourse(String ParameterNValue) throws Exception
	{
		try {
			...
			String strDescription = "IRT Tool";
			...
			final int IRTToolAsignment = 4;
			final int IRTToolCourse=5;
			...
			//add Course
			this.strDTParametersNValues="InputDataRow=>"+IRTToolCourse;
			addCourse();
			
			// Create Assignment with Policy
			this.strDTParametersNValues="InputDataRow=>"+IRTToolAsignment;
			createAssignment(IRTToolAsignment);
	...
	public void StudentTakingAssignment(String ParameterNValue) throws Exception
	{
		...
		//Q2
		connect().TakeAssignmentPage().answerFewOptionsQuestionTwo();
		submitAndCancel();
		verifyQuestionMapWithoutAnswering();
		
		checkMyWork();
		connect().TakeAssignmentPage().checkMyWorkValidation("Answer is not complete.");
		connect().TakeAssignmentPage().closeQuestionMap();
		connect().TakeAssignmentPage().nextQuestion();
		
		//Q3
		connect().TakeAssignmentPage().answerThirdQuestion();
		
		goToPreviousQuestion();
		connect().TakeAssignmentPage().clickQuestionMap();
		connect().TakeAssignmentPage().checkStatusForQuestion("answered",3);
		connect().TakeAssignmentPage().nextQuestion();
		connect().TakeAssignmentPage().deleteAnswersQuestionThreeAndCheckStatus();
		goToPreviousQuestion();
		connect().TakeAssignmentPage().clickQuestionMap();
		connect().TakeAssignmentPage().checkStatusForQuestion("incomplete",3);
		connect().TakeAssignmentPage().nextQuestion();
		connect().TakeAssignmentPage().answerThirdQuestionDeletedPart();
		checkMyWork();
		connect().TakeAssignmentPage().checkMyWorkValidation("Answer is complete and correct.");
		
		goToPreviousQuestion();
		goToPreviousQuestion();
		
		//Q1 answer  Incorrect Ans
		connect().TakeAssignmentPage().answerFirstQuestion();
		checkMyWork();
		connect().TakeAssignmentPage().checkMyWorkValidation("Answer is complete but not entirely correct.");
