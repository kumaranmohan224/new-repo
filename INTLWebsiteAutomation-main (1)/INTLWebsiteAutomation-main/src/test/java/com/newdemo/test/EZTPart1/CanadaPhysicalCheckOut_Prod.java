...
public class CanadaPhysicalCheckOut_Prod extends ScriptBase {
	...
	public void IntructorArea(String ParameterNValue, String env) throws Exception {
		try {
			...
			String strDescription = "Launch the Canada URL in browser for CanadaPhysicalCheckOut_Prod";
	...
	public void loginLATAMURL() throws Exception {
		...
		connect().INTLLoginPage().clickActionPrintPrimaryToCartPhysicalCheckOutCanada_Prod();
		connect().INTLLoginPage().clickAcceptAllButton();
		...
		connect().INTLLoginPage().clickShippingAddressLine2StreetTextBox();
		connect().INTLLoginPage().typeShippingAddressLine2Street();
		...
		connect().INTLLoginPage().selectShippingAddressRegionIdValueCanada();
